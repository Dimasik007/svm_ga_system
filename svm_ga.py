...
5) save model with pickle
    GA
* implement leverage activation function from x2 to x10 
1) train 3 Genetic Algorithms for different market types
2) 
    Backtesting
...
# to label market type based on condition
...
# trading rules per technical indicators
...
X = data_train.drop(['mType', 'mType1', 'Close', 'Volume'], axis=1)  # data price sequences type 1
...
# validation set
data_val = EUR_USD['2015-01-01':]
y_val = data_val['mType']  # target
...
print("Training data: {} and target: {}".format(X_test.shape, y_test.shape))
...
clf = GridSearchCV(SVC(class_weight={-1: 3.74030354, 0: 0.40926285, 1: 3.45752143}),
                   ...
                   n_jobs=-1, refit=True, return_train_score=False, verbose=100)
